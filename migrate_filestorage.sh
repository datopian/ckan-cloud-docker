...
BUCKET=${4}
STOREGE_PREFIX=${5}/
TMP_DATA_RECIEVED='/tmp/data_recieved'
TMP_DATA='/tmp/data'
...
./mc cp --recursive filestorage/$BUCKET/$STOREGE_PREFIX $TMP_DATA_RECIEVED

echo updating paths
for dir in $(ls $TMP_DATA_RECIEVED/resources); do
  FIRST_DIR=${dir:0:3}
  SECOND_DIR=${dir:3:3}
  REST=${dir:6}
  RESOURCE_DIR=$TMP_DATA/resources/$FIRST_DIR/$SECOND_DIR
  mkdir -p $RESOURCE_DIR
  for file in $TMP_DATA_RECIEVED/resources/$dir/*; do
    echo $file
    cp $file $RESOURCE_DIR/$REST
  done
done
cp -r $TMP_DATA_RECIEVED/storage $TMP_DATA/storage

echo mounting data into the persistant volumes
